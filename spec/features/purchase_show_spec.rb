...
RSpec.feature 'PurchaseShows', type: :feature do
  ...
  let!(:category) { Category.create(name: 'Groceries', icon: '/uploads/icon.png', user: user) }
  let!(:purchase) { Purchase.create(name: 'Milk', amount: 2.99, user: user) }

  before do
    login_as(user, scope: :user)
    PurchaseCategory.create(purchase: purchase, category: category)
